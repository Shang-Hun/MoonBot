    ...
    async def help(self, ctx, cmd=None):
        ...
        if ctx.message.content.endswith("help"):
            if cmd is not None:
                ...
                embed1.add_field(name="🏆Leveling", value="`rank` `leaderboard` `enabled` `disabled`", inline=False)
                embed1.add_field(name="🌎General", value="`ping` `help` `botinfo` `report`", inline=False)

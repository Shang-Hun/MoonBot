...
class Log(commands.Cog):
    ...
    async def on_guild_join(self, guild):
        ...
        if result is None:
            ...
            main.commit()
        if result2 is None:
            sql = ("INSERT INTO settings(guild_id, log_channel, welcome, leave, prefix, level) VALUES(?,?,?,?,?,?)")
            val = (str(guild.id), None, None, None, '>', None)
            cursor1.execute(sql, val)
            setting.commit()
        for channel in guild.text_channels:
            if channel.permissions_for(guild.me).send_messages:
                await channel.send(f'__**It\'s currently a beta version, so it\'s normal to have bugs!**__\nHi, i am {self.bot.user}! If you want to enable levelsys, please enter `>enabled` `[Need administrator permissions]`\nFor more help, please enter `>help`')
            break
        cursor.close()
        main.close()
        cursor1.close()
    ...
    async def on_member_join(self, member):
        ...
        setting = sqlite3.connect('db/setting.db')
        cursor1 = setting.cursor()
        cursor1.execute(f"SELECT welcome, level FROM settings WHERE guild_id = '{member.guild.id}'")
        result2 = cursor1.fetchone()
        if str(result2[1]) is None:

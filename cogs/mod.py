...
class Mod(commands.Cog):
    ...
    @commands.command(name="enabled", description='Enabled levelsys on this server', usage='>enabled')
    @commands.has_permissions(administrator=True)
    async def enabled(self, ctx):
        ...
        if str(result2[0]) == 'enabled':
            await ctx.send(f'this server already enabled levelsys!')
        else:
            sql = ("UPDATE settings SET level = ? WHERE guild_id = ?")
            val = ('enabled', str(ctx.guild.id))
            cursor1.execute(sql, val)
            setting.commit()
            await ctx.send(f'enabled levelsys on this server!')
        cursor1.close()
        setting.close()

    @commands.command(name="disabled", description='Disabled levelsys on this server', usage='>enabled')
    @commands.has_permissions(administrator=True)
    async def disabled(self, ctx):
        ...
        cursor1.execute(f"SELECT level FROM settings WHERE guild_id = '{ctx.guild.id}'")
        result2 = cursor1.fetchone()
        if str(result2[0]) == 'None':
            await ctx.send(f'this server already disabled levelsys!')
        else:
            if result2 is not None:
                ...
                cursor1.execute(sql, val)
                setting.commit()
                await ctx.send(f'disabled levelsys on this server!')
        cursor1.close()
        setting.close()

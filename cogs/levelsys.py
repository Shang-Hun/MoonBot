...
class TextLeveling(commands.Cog, name='Leveling'):
    ...
    async def on_message(self, message):
        if message.author.bot:
            ...
            cursor1.execute(f"SELECT level FROM settings WHERE guild_id = '{message.guild.id}'")
            result2 = cursor1.fetchone()
            if str(result2[0]) is None:
                return
            elif str(result2[0]) == 'enabled':
                if result is None:
                    return
    ...
    @commands.command(name="rank", description='View the level rank of the member', usage='>rank [member]')
    @commands.guild_only()
    async def rank(self, ctx, user:discord.Member=None):
        ...
        setting = sqlite3.connect('db/setting.db')
        cursor1 = setting.cursor()
        cursor1.execute(f"SELECT level FROM settings WHERE guild_id = '{ctx.guild.id}'")
        result2 = cursor1.fetchone()
        if str(result2[0]) is None:
            return
        elif str(result2[0]) == 'enabled':
            ...
            #     await ctx.send(f'{ctx.author.mention} this server\'s levelsys is disabled!')
                ...
                if user is None:
                    ...
                    if result is None:
                        ...
                        # Working with Fonts
                        big_font = ImageFont.FreeTypeFont("db/setofont.otf", 55)
                        medium_font = ImageFont.FreeTypeFont("db/setofont.otf", 35)
                        small_font = ImageFont.FreeTypeFont("db/setofont.otf", 25)

                        # Placing Right Upper Part
                        text_size = draw.textsize(str(lvl), font=big_font)
                        offset_x = 1000 - 15 - text_size[0]
                        offset_y = 10
                        draw.text((offset_x, offset_y), str(lvl), font=big_font, fill="#f4ec90")

                        text_size = draw.textsize("LEVEL", font=small_font)
                        offset_x -= text_size[0] + 5
                        draw.text((offset_x, offset_y + 27), "LEVEL", font=small_font, fill="#f4ec90")
                        ...
                        # Progress Bar
                        draw.rectangle((bar_offset_x, bar_offset_y, bar_offset_x_1, bar_offset_y_1), fill="#f4ec90")

                        # Left Circle
                        draw.ellipse(
                            (bar_offset_x - circle_size // 2, bar_offset_y, bar_offset_x + circle_size // 2, bar_offset_y_1), fill="#f4ec90"
                        )

                        # Right Circle
                        draw.ellipse(
                            (bar_offset_x_1 - circle_size // 2, bar_offset_y, bar_offset_x_1 + circle_size // 2, bar_offset_y_1), fill="#f4ec90"
                        )
    ...
    @commands.command(name="leaderboard", description='View the ranking of the server', usage='>leaderboard | >lb', aliases=['lb'])
    @commands.guild_only()
    async def leaderboard(self, ctx):
        ...
        if str(result2[0]) is None:
            ...
            #     await ctx.send(f'{ctx.author.mention} this server\'s levelsys is disabled!')
                ...
                main = sqlite3.connect('db/main.db')
                cursor = main.cursor()
                cursor.execute(f"SELECT user_id, exp, level FROM glevel WHERE guild_id = '{ctx.guild.id}' ORDER BY level + 0 DESC, exp + 0 DESC")
                result = cursor.fetchall()
